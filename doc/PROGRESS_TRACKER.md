...
| ----------------- | ---------------- | --------------------------------------------------------------------------------------------- |
| Version           | 0.4.0            | `pubspec.yaml`, `CHANGELOG.md`.                                                               |
...
| ML-KEM            | Supported        | Checked-in KAT corpus, unit tests, OpenSSL/liboqs interop.                                    |
| Native interop    | Supported        | OpenSSL and liboqs tool suites cover ML-KEM, ML-DSA, and SLH-DSA outside the runtime package. |
...
| SLH-DSA           | FIPS 205-aligned | All 12 sets (SHAKE + SHA-2) shipped in 0.4.0; 1,248/1,248 ACVP cases byte-exact.              |
...
| Rewrite stale 0.1.0-era docs.                       | Done   | Evidence-scoped 0.4.0 docs.                 |
...
| Maintain native interop for ML-KEM/ML-DSA/SLH-DSA. | Open   | `tool/openssl_interop` with OpenSSL 4.0.1; `tool/liboqs_interop` with liboqs 0.15.0. |
...
| ---------- | ----------------- | ------------------------------------------------------------ |
| SLH-DSA    | 0.4.0 (all 12)    | Hash-based; SHAKE + SHA-2 byte-exact on ACVP. See the guide. |
...
and ship together in 0.4.0. SHAKE was implemented
...
| Published VM/dart2js/dart2wasm SHAKE benchmark baselines.   | Done   | 18 SHAKE measurements published in `PERFORMANCE.md`.     |
| All-set benchmark harness.                                  | Done   | `tool/bench/slhdsa_bench.dart` supports all 12 sets.     |
| Full VM and web package platform matrix.                    | Done   | VM decomposed matrix; 217/217 on dart2js and dart2wasm.  |
| Visibility and AI-discovery release boundary.               | Done   | Generated surfaces present SLH-DSA as released in 0.4.0. |
| Package publication preflight.                              | Done   | Valid archive; expected dirty-worktree warning only.     |
| Maintainer v0.4.0 tag/publish decision.                     | Open   | 0.4.0 metadata done; tag/publish are maintainer actions. |
...
- `dart run tool/visibility/generate_visibility.dart --check` passed after the
  ...
  sets, 1,248/1,248 evidence, and the 0.4.0 release boundary;
...
The 0.4.0 version is set; cutting the release branch, tag, and publication
