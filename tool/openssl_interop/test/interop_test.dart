...
void main() {
  ...
  final ownedKeys = <Pointer<EVP_PKEY>>[];

  setUpAll(() {
    printOnFailure('libcrypto: $libPath — ${ossl.version()}');
  });

  // Track an EVP_PKEY for teardown so a failing expectation never leaks it.
  Pointer<EVP_PKEY> own(Pointer<EVP_PKEY> k) {
    ownedKeys.add(k);
    return k;
  }
  ...
  for (final level in mlKemLevels) {
    ...
    group(name, () {
      test('sizes match FIPS 203 (OpenSSL & pqcrypto, pk/ct/sk/ss)', () {
        ...
        expect(
          osslPub,
          hasLength(level.specPublicKeyBytes),
        ...
        expect(
          osslCt,
          hasLength(level.specCiphertextBytes),
          reason: 'OpenSSL ciphertext size',
        );
        expect(
          osslSs,
          hasLength(level.specSharedSecretBytes),
        ...
        expect(pqPub, hasLength(level.specPublicKeyBytes));
        expect(pqSk, hasLength(level.specSecretKeyBytes));
        expect(pqCt, hasLength(level.specCiphertextBytes));
        expect(pqSs, hasLength(level.specSharedSecretBytes));
      });
      ...
      test(
        ...
        () {
          for (var i = 0; i < _fuzzIterations; i++) {
            ...
            expect(ssAlice, hasLength(kSharedSecretBytes));
          }
        },
      );
      ...
      group(
        ...
        () {
          ...
          test('G: implicit-rejection secret J(z‖c) agrees on an invalid ct', () {
            ...
            expect(ssRejPq, hasLength(kSharedSecretBytes));
            expect(
              ssRejOssl,
              equals(ssRejPq),
...
Uint8List _seed(MlKemLevel level, {required int salt}) {
  final seed = Uint8List(kSeedBytes);
  for (var i = 0; i < seed.length; i++) {
    seed[i] = (i * 7 + level.pq.params.k * 31 + salt * 101) & 0xFF;
  }
  return seed;
}
