...
- SLH-DSA: in development. Algorithms 1-25 are implemented for all 12 parameter
  sets (SHAKE and SHA-2 families), and `test/slhdsa_kat_test.dart` is byte-exact
  on the 1,248 cases in the pinned official NIST ACVP sample corpus. The external
  API is exported in the development tree; verify-after-sign, BUFF/performance
  docs, and VM/dart2js/dart2wasm benchmark baselines are complete. The decomposed
  VM matrix, both web compiler suites, and publication preflight are green; the
  v0.4.0 version, tag, and publication are not yet done. This is KAT/regression
